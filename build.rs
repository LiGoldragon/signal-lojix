...
const MODULE_PATH: &[&str] = &["signal_lojix", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        .expect("assemble authority-approved ordinary Lojix Interface transaction");
        let rust = rust_logos();
        let type_paths = LojixRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project ordinary Lojix Interface from the verified transaction")
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("Lojix bootstrap catalog is complete")
}
...
struct LojixRustTypePaths(BTreeMap<VocabularyEncodedId, RustTypePath>);

impl LojixRustTypePaths {
    fn new() -> Self {
        use bootstrap_manifest as manifest;
...
impl RustTypePathResolver for LojixRustTypePaths {
